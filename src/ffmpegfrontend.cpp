...
*  Copyright (C) 2010-2012  Suraj Kiran - spidi123q@gmail.com         *
*                                                                          *
    ...
    ui(new Ui::ffmpegFrontend)
{
    ...
    ui->CB_container_2->setStyleSheet("font: 75 9pt \"Nimbus Mono L\";");
...
void ffmpegFrontend::connectSlots()
{
    ...
    connect(ui->CB_container_2, SIGNAL(currentIndexChanged(int)), this, SLOT(on_CB_container_currentIndexChanged(int)) );
...
void ffmpegFrontend::disconnectSlots()
{
    ...
    b=disconnect(ui->CB_container_2, SIGNAL(currentIndexChanged(int)) ,0 ,0 );
...
void ffmpegFrontend::setDefaults()
{
    ...
    ui->CB_container_2->setCurrentIndex(0);
...
QFileInfoList ffmpegFrontend::getPresetFileList()
{
    ...
    // search ffmpeg-gui directory
        ...
        ffpresetDir.setPath("/usr/lib/icefeast/outreel/");
        ffpresetDir=ffpresetDir.toNativeSeparators(ffpresetDir.absolutePath());
//        if (ffpresetDir.exists()) qDebug() << "ffmpeg presets dir: " << ffpresetDir.absolutePath();
        local_preset_directory = ffpresetDir;
    #else
...
void ffmpegFrontend::setupCB_formats()
{
    ...
    ui->CB_container_2->addItem(tr("don't care"), "don't care");
    ...
    while ((pos = rx.indexIn(result, pos)) != -1)
    {
        ...
        if (format.contains("E"))
        {
            ...
            ui->CB_container_2->addItem(item_text, description_words.at(0));
            ...
            if (description_words.at(0).contains("dvd", Qt::CaseInsensitive) ||
                ...
                ui->CB_container_2->setItemIcon(ui->CB_container_2->count()-1, QIcon(":/icons/icons/circle.png"));
            else
                ui->CB_container_2->setItemIcon(ui->CB_container_2->count()-1, QIcon(":/icons/icons/transparent.png"));
        }
...
void ffmpegFrontend::onCB_ffmpeg_preset_changed(int i)
{
    ...
    if (i==0) {
        ...
        if (ui->CB_ffmpeg_preset->itemData(i).toString().contains("/usr/lib/icefeast/outreel/") || // This if we are in release mode
            ui->CB_ffmpeg_preset->itemData(i).toString().contains("/usr/lib/icefeast/outreel/"))   // This if we are in debug mode
        {
            ...
            for (int n=0; n<ui->CB_video_codec->count(); n++)
                ...
                if (codec=="libx264") {
                    ui->CB_container_2->setCurrentIndex(ui->CB_container_2->findText("mp4",Qt::MatchStartsWith));
                    check_command("-f","mp4");
...
void ffmpegFrontend::on_CB_container_currentIndexChanged(int index)
{
    ...
    if (index==0) {
        ...
        check_command("-f",ui->CB_container_2->itemData(index).toString());
        update_ffmpeg_args();
    }
}
...
void ffmpegFrontend::update_ffmpeg_GUI_args()
{
    ...
    for (i=0; i<commands.size(); i++)
    {
        ...
        if (command=="f") {
            n=ui->CB_container_2->findData(commands_string.at(i), Qt::UserRole, Qt::MatchStartsWith);
            if (n!=-1) {
                ui->CB_container_2->setCurrentIndex(n);
            }
            else ui->CB_container_2->setCurrentIndex(0);
        }
...
void ffmpegFrontend::runFfmpeg(QStringList args, QFileInfo input_file, QFileInfo output_file)
{
    ...
    QString ext = ui->CB_container_2->itemData(ui->CB_container_2->currentIndex()).toString();
    if (ui->CB_container_2->currentIndex() == 0) // If "don't care" is set in CB_container_2
        ext="";
    ...
    // add to output_file the extension format from the one choosen in CB_container_2
    if (ui->CB_container_2->currentIndex() > 0)
    {
        // Check if it has the right ext
        if (!output_file.completeSuffix().contains(ui->CB_container_2->itemData(ui->CB_container_2->currentIndex()).toString() ) )
            output_file.setFile(output_file.absolutePath(), output_file.baseName()+"."+
                                ui->CB_container_2->itemData(ui->CB_container_2->currentIndex()).toString());
    }
